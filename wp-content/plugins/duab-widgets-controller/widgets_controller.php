<?php
/*
Plugin Name: DUAB - Widget Controller
Plugin URI: http://dotunited.se
Description: En plugin för att visa widgets specifikt för kategori och/eller sidor.
Author: Dot United AB
Author URI: http://dotunited.se
Version: 0.1
*/
global $post;
global $wp_query;
...
function widgets_controller_show($instance) {
	if($instance['widgets_controller'] == 1) {
		global $wp_query;
		$categorylist = $instance['category'];
		...
		if(is_home() || is_front_page()) {
			...
			if ($instance['category']) {
				...
				$cat_id = get_query_var('cat');
				if (in_array($category[0]->category_parent, $instance['category']) || in_array($cat_id, $instance['category'])) {
					return $instance;
				} else {
					return false;
				}
		...
		} elseif(is_single()) {	// DUAB, we removed posts but check post categoryid to match catagories..
			if($instance['category']) {
				global $wp_query;
				$post_obj = $wp_query->get_queried_object();
				$Page_ID = $post_obj->ID;
				$category = get_the_category($Page_ID);
				if(in_array($category[0]->cat_ID, $instance['category'])) {
					return $instance;
				} else {
					return false;
				}
...
function widgets_controller_append($widget, $return, $instance) {
	...
	?>
	<p><input for_id="<?php echo $widget->get_field_id(""); ?>" get_cat_name="<?php echo $widget->get_field_name("category"); ?>" get_pages_name="<?php echo $widget->get_field_name("pages"); ?>" current_cat="<?php echo implode("," , $instance['category']); ?>" current_pages="<?php echo implode("," , $instance['pages']); ?>" class="checkbox widgets_controller" type="checkbox" <?php checked($instance['widgets_controller'], true) ?> id="<?php echo $widget->get_field_id('widgets_controller'); ?>" name="<?php echo $widget->get_field_name('widgets_controller'); ?>" value="1" /><label class="manage_label" for="<?php echo $widget->get_field_id('widgets_controller'); ?>"><?php _e('Widget-kontroll', 'display-widgets') ?></label></p>
	<div class="widgets_controller_box<?php echo $none; ?>"><span class="show_detail">Detaljer</span>
		<?php //echo "<pre>"; print_r($widget); print_r($instance); echo "</pre>"; ?>
		<div><span><input type="checkbox" <?php if($instance['general']) checked(in_array('homepage', $instance['general']), true) ?> id="<?php echo $widget->get_field_id('homepage'); ?>" name="<?php echo $widget->get_field_name('general'); ?>[]" value="homepage"><label for="<?php echo $widget->get_field_id('homepage'); ?>"><?php _e('Förstasidan', 'display-widgets') ?></label></span><span><input type="checkbox" <?php if($instance['general']) checked(in_array('error', $instance['general']), true) ?> id="<?php echo $widget->get_field_id('error'); ?>" name="<?php echo $widget->get_field_name('general'); ?>[]" value="error"><label for="<?php echo $widget->get_field_id('error'); ?>"><?php _e('404-Felsida', 'display-widgets') ?></label></span><span><input type="checkbox" <?php if($instance['general']) checked(in_array('search', $instance['general']), true) ?> id="<?php echo $widget->get_field_id('search'); ?>" name="<?php echo $widget->get_field_name('general'); ?>[]" value="search"><label for="<?php echo $widget->get_field_id('search'); ?>"><?php _e('Söksidan', 'display-widgets') ?></label></span></div>
...
function widgets_controller_submit() {
	...
	$ajax_data = "<p>Kategori</p><div class='overflow category_box'>";
	$cat_list = $wpdb->get_results( "SELECT ".$prefix."terms.term_id, ".$prefix."terms.name FROM ".$prefix."term_taxonomy INNER JOIN ".$prefix."terms ON ".$prefix."term_taxonomy.term_id = ".$prefix."terms.term_id WHERE ".$prefix."term_taxonomy.taxonomy='category' ORDER BY ".$prefix."terms.name ASC" );
	foreach($cat_list as $k => $v) {
		$for_id = $v->term_id . 0;
		$title = substr($v->name, 0, 20);
		$checked = in_array($v->term_id, $current_cat) ? "checked" : "";
		$ajax_data .= "<span><input {$checked} type='checkbox' id='{$data["for_id"]}{$for_id}' name='{$data["get_cat_name"]}[]' value='{$v->term_id}'><label for='{$data["for_id"]}{$for_id}'>".$title."</label></span>";
	} 
	...
	$ajax_data .= "</div><p>Sidor</p><div class='overflow pages_box'>";
	$page_list = $wpdb->get_results( "SELECT * FROM ".$prefix."posts WHERE post_type='page' && post_status='publish' ORDER BY post_title DESC" );
	foreach($page_list as $k => $v) {
		...
		$ajax_data .= "<span><input {$checked} title='{$parent_id}' type='checkbox' id='{$data["for_id"]}{$for_id}' name='{$data["get_pages_name"]}[]' value='{$v->ID}'><label for='{$data["for_id"]}{$for_id}'>{$title}</label></span>";
	}
	$ajax_data .= "</div>";
	echo $ajax_data;
exit;
